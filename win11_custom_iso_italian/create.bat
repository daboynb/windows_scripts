...
IF %errorlevel% equ 0 (
  ...
  color 4 && echo "ERROR: Impossibile eliminare KB5029263!" && pause && del "resources\unattend_edited.xml" /q && rmdir "C:\mount" /s /q && rmdir "C:\ISO" /s /q && exit /b 1
)
cls
...
:delete_iso
set /p answer="Rimuovere le ISO originali di microsoft ? (si/no): "
if /i "%answer%"=="si" (
del "%filepath%" /q
del "%iso11%" /q
) else if /i "%answer%"=="no" (
